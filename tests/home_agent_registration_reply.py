        ...
        try:
            for packet in pcap_file:
                ...
                if dst_addr == self._dest_addr:
                    print("\nForeign agent received registration reply message from Mobile Node on its IP address %s as expected\n" % dst_addr)
                    state.append(True)
                else:
                    print("\nRegistration reply message is Not sent to the Foreign agent IP address %s but to another destination address %s -- Test Failed\n"% (self._dest_addr, dst_addr))
                    state.append(False)
